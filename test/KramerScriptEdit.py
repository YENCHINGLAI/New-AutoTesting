...
class TestStep:
    def __init__(self, line_number, name, limits, unit, delay, repeat, critical, comments=""):
        ...
        with open(filename, 'r') as f:
            ...
            while i < len(lines):
                ...
                if line.startswith('Model'):
                    i+=1
                    continue
                if line.startswith('UseMAC'):
                    i+=1
                    continue
                if line.startswith('UNIT_ver'):
                    i+=1
                    continue   
                if line.startswith('Pairing'):
                    i+=1
                    continue
                if line.startswith(':'): #Test Item
                    try:
                      parts = line.split('.', 1)
                      ...
                      while i < len(lines):
                         ...
                         if current_line.startswith("Limits"):
                           ...
                           break
                         i+=1
                      test_steps.append(TestStep(line_number, name, limits, unit, delay, repeat, critical))
...
class TestEditor:
    def __init__(self, root):
        ...
        self.listbox = tk.Listbox(root, width=setWidth, height=20)
        ...
        self.line_number_entry = ttk.Entry(self.edit_frame, width=setWidth)
